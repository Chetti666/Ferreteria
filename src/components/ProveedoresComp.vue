<template>
  <LayouT>
    <div>
      ...
      <v-container>
        <v-card class="mx-auto rounded-lg mt-5" max-width="1200" outlined color="lightblue">
          <v-card-title class="text-h5 font-weight-bold">Lista de Proveedores</v-card-title>
          <v-card-text>
            <v-data-table
              :headers="headersProveedores"
              :items="proveedoresActuales"
              :items-per-page="5"
              class="elevation-1"
            >
              <template v-slot:item.contacto="{ item }">
                <v-chip color="primary" text-color="white">{{ item.contacto }}</v-chip>
              </template>
              <template v-slot:item.telefono="{ item }">
                <v-chip color="secondary" text-color="white">{{ item.telefono }}</v-chip>
              </template>
            </v-data-table>
          </v-card-text>
        </v-card>
...
export default {
  ...
  data() {
    return {
      paginaHomes: [],
      promociones: [
        { descripcion: "Promoción 1: 10% de descuento en productos seleccionados.", fechaInicio: "2023-10-01", fechaFin: "2023-10-15" },
        { descripcion: "Promoción 2: Compra uno y llévate otro a mitad de precio.", fechaInicio: "2023-10-05", fechaFin: "2023-10-20" },
        { descripcion: "Promoción 3: Envío gratis en compras mayores a $50.", fechaInicio: "2023-10-10", fechaFin: "2023-10-30" }
      ],
      secciones: [
  ...
  methods: {
    async fetchHomeData() {
      try {
        ...
        this.proveedoresActuales = data.paginaHome.proveedoresActuales;
        this.paginaHomes = [data.paginaHome];
        this.redSocial = data.paginaHome.informacionGeneral.redesSociales;
      } catch (error) {
        console.error("Error al obtener los datos:", error);
        alert("No se pudieron cargar los datos. Intenta de nuevo más tarde."); // Mensaje al usuario
      }
    },
  },
  mounted() {
